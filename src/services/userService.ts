...
export const updateTodo = async (todoId: number, todoData: any, userId: number): Promise<Todo> => {
  ...
  try {
    ...
    // Check if todo belongs to user and get old data
    const [todos] = await connection.execute(
      'SELECT id, start_time FROM todos WHERE id = ? AND user_id = ?',
      [todoId, userId]
    );
    ...
    const oldTodo = (todos as any[])[0];
    const oldStartTime = oldTodo.start_time ? DateTime.toTimestamp(oldTodo.start_time) : null;
    ...
    if (todoData.start_time !== undefined) {
      updateFields.push('start_time = ?');
      values.push(todoData.start_time);
      
      // Reset notification flag if start_time changed
      const newStartTime = todoData.start_time ? DateTime.toTimestamp(todoData.start_time) : null;
      if (oldStartTime !== newStartTime) {
        updateFields.push('start_notification_sent = FALSE');
      }
    }
    
    if (todoData.end_time !== undefined) {
